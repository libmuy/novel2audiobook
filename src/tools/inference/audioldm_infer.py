...
运行环境：独立的 audioldm venv，不与项目主 venv/src/tools/inference 环境混用
（理由同 docs/indextts_setup.md）。由 src/pipeline/asset_gen.py 的 AudioLDMBackend 通过
子进程调用，协议与 src/tools/inference/tangoflux_infer.py 一致（单次加载、批量循环、
...
用法：
    python src/tools/inference/audioldm_infer.py \
